...
One challenge that emerges as a result of subsetting sites is being able to compare new data with historical data. We therefore assumed that one goal of subsetting sites is to have a collection of sites that is representative of the entire project footprint in terms of species composition and diversity. We used a similarity index to evaluate how representative counts in a subset are to counts in the entire sample. This similarity assessments was implemented using the bioenv function in the Vegan package (Oksanen et al. 2019) in R. We selected a set of sites with a rho value of at least 0.95 when compared with the full set of sites in terms of species composition and relative abundance. The community was represented by a small number of ponds (<10), so we manually added ponds to the sample based on these user-defined criteria: (1) site accessibility; (2) inclusion of particular sites of interest, comprising ...; and (3) inclusion of at least two sites matching each of these types: (a) managed pond, (b) restored pond, (c) pond planned for restoration, and (d) pond managed for Western Snowy Plover breeding.
...
#FIGURES
```{r, echo=FALSE}
knitr::include_graphics(path="fig.RUDU.2surveys.png")
